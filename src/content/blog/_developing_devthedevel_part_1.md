...
The goal of this blog is to hopefully educate and entertain newer software developers. Another goal is to help me build up my own site and portfolio. And yet another goal is to keep me busy (is it possible to get any more busy than what I currently am?). And even yet another goal is to motivate me to actually stick to a project and learn new things (instead of working on a project for a couple days and then throwing it into the abyss that is my 'backlog' 😩).

So lets get started!

Oh one last note: I'm writing this about 6 months _after_ I originally created the site. I forget a lot of things, but I'll do my best to remember all the significant details. Going forward, blog posts should be released as I'm working on changes.
...
- And finally it has lots of integrations! If I ever wanted to add more complex features, I can easily add React to the site. Or TailwindCSS. Or whatever I like!

This is a winning combination for a simple blog website.

#### Back to the Project for an Astro Crash Course
...
- `assets/` is obviously where I keep any assets
- `components/` is where I keep Astro components (they work almost identical to React components)
- `content/` is where I keep any website collections. More information about this later
- `layouts/` is where I keep layouts, which are essentially Astro components that are meant to be used as reusable templates (like this blog page you are reading right now)
- `pages/` is where I define new pages for the site. More information about this later as well
- `styles/` is where all the site styles go

What makes Astro so great for static content sites is how it handles pages. Pages are simply markdown files within `pages/` (or they can be Astro components). Astro loads any files within `pages/` and generates a page for each one that can be accessed like `website_url/page_name`. For example, my About page is stored as `pages/about.astro` and can be accessed from `devthedevel.xyz/about`. Neat eh?

Hold up, whats that `.astro` file extension? Thats an Astro component. These components have two sections to them: a script section and a template section. The script section is where you can run code _before_ Astro generates the final page. You can import other components, data, or even fetch content from a server. You can also create variables here that can be used in the template.
...
_Ok Devin you obviously have something in mind so just tell me already_. I will, jeez calm down reader. Astro supports dynamic routing: all you need to do is name the file correctly and tell Astro how to generate the static paths for a page.
...
Although its very very very unlikely to see this page, I kinda love it. Makes me feel like a real web developer (my entire body shuddering typing that).

And that basically catches you up to where I am as I'm typing.

#### Whats Next?

I now have a working site that has content 🎉! I could end leave this project as is, and only write new blog posts. But if I planned on doing that, then why did I name this post 'Developing devthedevel.xyz: _Part 1_ '?
...
- Adding an avatar to the header
